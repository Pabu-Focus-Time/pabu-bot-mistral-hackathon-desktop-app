...
def _automodel_from_pretrained(model_id_or_path: str, **kwargs):
    ...
    if token:
        try:
            return AutoModel.from_pretrained(model_id_or_path, token=token, **kwargs)
        except TypeError:
            return AutoModel.from_pretrained(model_id_or_path, use_auth_token=token, **kwargs)
...
def infer_stride_and_special_tokens(H: int, W: int, T: int, prefer_ps: int) -> Tuple[int, int]:
    """
    Infer token grid stride (patch size / feature stride) from model output length.
    Works for both ViT-like (stride=patch_size) and ConvNeXt-like backbones (stride often 32).
    Returns: (ps, n_special)
    """
    # Try the preferred ps first, then common candidates.
    ...
    # DINOv3 models may include CLS + register tokens; keep this small but flexible.
    ...
    # Fallback: assume at least 1 special token and keep prefer_ps.
    return prefer_ps, 1
...
def load_model_cached(full_model_id: str, device_str: str):
    ...
    trust = bool(str(full_model_id).startswith("facebook/dinov3")) or os.path.isdir(str(full_model_id))
    model = _automodel_from_pretrained(full_model_id, trust_remote_code=trust).to(device)
    model.eval()
    return model
...
class PatchImageState:
    def __init__(self, pil_img: Image.Image, model, device_str: str, ps: int, dataset_key: str):
        ...
        # Pad to a safe multiple. ConvNeXt-style backbones commonly stride by 32.
        inputs, disp_np, _ = preprocess_no_resize(pil_img, multiple=max(int(ps), 32), dataset_key=dataset_key)
        self.disp = disp_np
        ...
        if T - n_special != n_patches:
            raise RuntimeError(
                f"Token mismatch: T={T}, inferred n_patches={n_patches}, HxW={self.H}x{self.W}, ps={self.ps}"
            )
        self.D = D
        patches = hs[n_special:n_special + n_patches, :].reshape(self.rows, self.cols, D)
...
def init_states(
    ...
    # Resolve model (either dropdown selection or custom ID/path)
    custom = (custom_model_id_or_path or "").strip()
    if custom:
        full_model_id = custom
    else:
    ...
    try:
        ...
        # Return a graceful error instead of crashing the ASGI response mid-stream.
        msg = (
            f"❌ Failed to build patch tokens for the selected model.\n\n"
            f"- **Model**: `{full_model_id}`\n"
            f"- **Error**: `{type(e).__name__}: {e}`\n\n"
            f"Tip: try `vits16` first; some backbones (e.g. ConvNeXt) use a different stride."
        )
        return (gr.update(), gr.update(), None, None, 0, -1, -1, int(ps), msg)
...
with gr.Blocks(title="DINOv3 Patch Similarity (Self & Cross)") as demo:
    ...
    custom_model = gr.Textbox(
        label="Custom model ID or local path (optional)",
        placeholder="Example: facebook/dinov3-vits16-pretrain-lvd1689m  OR  D:\\models\\dinov3-vits16-pretrain-lvd1689m",
    )
    ...
    init_btn.click(
        fn=init_states,
        inputs=[left_input, left_url, right_input, right_url, dataset_radio, model_dropdown, custom_model, show_grid, overlay_alpha],
