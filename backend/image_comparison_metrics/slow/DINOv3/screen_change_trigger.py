...
HISTORY_MAX = int(os.environ.get("DINO_CHANGE_HISTORY", "240"))
PLOT_YMAX = float(os.environ.get("DINO_PLOT_YMAX", "0.4"))
PLOT_W = int(os.environ.get("DINO_PLOT_W", "900"))
PLOT_H = int(os.environ.get("DINO_PLOT_H", "450"))
PLOT_TITLE_SCALE = float(os.environ.get("DINO_PLOT_TITLE_SCALE", "0.65"))
PLOT_LEGEND_SCALE = float(os.environ.get("DINO_PLOT_LEGEND_SCALE", "0.55"))
PLOT_FOOTER_SCALE = float(os.environ.get("DINO_PLOT_FOOTER_SCALE", "0.55"))

# Trigger controls
THRESH = float(os.environ.get("DINO_TRIGGER_THRESH", "0.15"))
CONSEC = int(os.environ.get("DINO_TRIGGER_CONSEC", "1"))
COOLDOWN_SEC = float(os.environ.get("DINO_TRIGGER_COOLDOWN_SEC", "5.0"))

# Alarm script controls
ALARM_MODE = os.environ.get("DINO_TRIGGER_MODE", "banner").strip().lower()  # banner|msgbox
ALARM_SECONDS = float(os.environ.get("DINO_TRIGGER_SECONDS", "2.0"))
...
def push_hist(hist, v: float):
    hist.append(float(v))
    if len(hist) > HISTORY_MAX:
        del hist[: len(hist) - HISTORY_MAX]


def render_plot(history, w: int, h: int, ymax: float):
    w, h = int(w), int(h)
    ymax = float(ymax) if float(ymax) > 0 else 0.4

    img = np.zeros((h, w, 3), dtype=np.uint8)
    cv2.putText(img, "dino_col_change over time", (10, 28), cv2.FONT_HERSHEY_SIMPLEX, PLOT_TITLE_SCALE, (255, 255, 255), 2, cv2.LINE_AA)

    x0, y0 = 10, 45
    x1, y1 = w - 10, h - 25
    cv2.rectangle(img, (x0, y0), (x1, y1), (120, 120, 120), 1)

    N = min(len(history), max(2, x1 - x0))
    series = history[-N:]
    pts = []
    for i, v in enumerate(series):
        if not np.isfinite(v):
            continue
        t = i / float(max(1, len(series) - 1))
        x = int(x0 + t * (x1 - x0))
        vv = max(0.0, min(float(v), ymax))
        y = int(y1 - (vv / ymax) * (y1 - y0))
        pts.append((x, y))
    if len(pts) >= 2:
        cv2.polylines(img, [np.array(pts, dtype=np.int32)], False, (140, 255, 140), 2)

    cv2.putText(
        img,
        f"ymax={ymax:g}  k={SCROLL_K}  thresh={THRESH:g}",
        (x0 + 8, y0 + 20),
    ...
    last = next((v for v in reversed(history) if np.isfinite(v)), float("nan"))
    cv2.putText(img, f"latest: {last:.3f}", (10, h - 6), cv2.FONT_HERSHEY_SIMPLEX, PLOT_FOOTER_SCALE, (220, 220, 220), 2, cv2.LINE_AA)
    return img


def spawn_alarm(change_value: float):
    alarm_path = os.path.join(os.path.dirname(__file__), "screen_change_alarm.py")
    text = f"trigger smth  (dino_col_change={change_value:.3f} > {THRESH:g})"
    try:
        subprocess.Popen(
            [
                sys.executable,
...
def main():
    win_name = "DINO screen dino_col_change (ESC to quit)"
    ...
    with mss.mss() as sct:
        ...
        while True:
            ...
            push_hist(history, change)
            prev_Xn = curr_Xn
            prev_shape = curr_shape

            # Print once/sec (useful to pipe to backend later)
            now = time.time()
            if now - last_print >= 1.0 and np.isfinite(change):
                print(f"dino_col_change={change:.4f}", flush=True)
                last_print = now
...
def render_plot(history, w: int, h: int, ymax: float):
    ...
    cv2.putText(img, f"ymax={ymax:g}  k={SCROLL_K}  thresh={THRESH:g}", (x0 + 8, y0 + 20), cv2.FONT_HERSHEY_SIMPLEX, PLOT_LEGEND_SCALE, (200, 200, 200), 2, cv2.LINE_AA)
...
def main():
    ...
    with mss.mss() as sct:
        ...
        while True:
            frame = np.array(sct.grab(mon))[:, :, :3]  # BGR view
...
def spawn_alarm(change_value: float):
    ...
    try:
        ...
        # If spawning fails, at least log.
        print(text, flush=True)
...
TOKEN_STRIDE_PREFER = int(os.environ.get("DINO_TOKEN_STRIDE_PREFER", "16"))
SCROLL_K = int(os.environ.get("DINO_SCROLL_K", "16"))  # vertical search radius (in patch rows)
...
def render_plot(history, w: int, h: int, ymax: float):
    ...
    cv2.putText(img, f"ymax={ymax:g}  k={SCROLL_K}", (x0 + 8, y0 + 20), cv2.FONT_HERSHEY_SIMPLEX, PLOT_LEGEND_SCALE, (200, 200, 200), 2, cv2.LINE_AA)
...
def main():
    ...
    with mss.mss() as sct:
        ...
        while True:
            ...
            # Print occasionally (useful for piping to another process later)
            now = time.time()
            if now - last_print >= 1.0 and np.isfinite(change):
                print(f"dino_col_change={change:.4f}", flush=True)
                last_print = now
