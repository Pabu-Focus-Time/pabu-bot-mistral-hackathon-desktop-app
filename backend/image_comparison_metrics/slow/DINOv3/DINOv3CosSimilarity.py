...
# Load local `.env` if present (HF_TOKEN, etc.)
try:
...
def _automodel_from_pretrained(model_id_or_path: str, **kwargs):
    # transformers has used both `token=` and `use_auth_token=` across versions
    if HF_TOKEN:
        try:
            return AutoModel.from_pretrained(model_id_or_path, token=HF_TOKEN, **kwargs)
        except TypeError:
            return AutoModel.from_pretrained(model_id_or_path, use_auth_token=HF_TOKEN, **kwargs)
...
# ---------- Image I/O ----------
def _maybe_resize(pil_img: Image.Image, max_side: int | None):
    if not max_side:
        return pil_img
    ...
    return pil_img.resize((nw, nh), resample=Image.BICUBIC)

def load_image(path_or_url, max_side: int | None = None):
    ...
    return _maybe_resize(img, max_side=max_side)
...
class PatchImageState:
    def __init__(self, pil_img, model, device, ps):
        self.pil = pil_img
        self.ps = ps
        inputs, disp_np, _ = preprocess_image_no_resize(pil_img, multiple=max(int(ps), 32))
        self.disp = disp_np
        ...
        T, D = hs.shape
        inferred_ps, n_special = infer_stride_and_special_tokens(self.H, self.W, T, prefer_ps=int(ps))
        self.ps = int(inferred_ps)
        self.rows, self.cols = self.H // self.ps, self.W // self.ps
        n_patches = self.rows * self.cols
        if T - n_special != n_patches:
            raise RuntimeError(
                f"[error] Token shape mismatch. T={T}, inferred n_patches={n_patches}, HxW={self.H}x{self.W}, ps={self.ps}"
            )

        self.D = D
        self.patch_embs = hs[n_special:n_special + n_patches, :].reshape(self.rows, self.cols, D)  # (rows,cols,D)
...
# ---------- Single-image mode ----------
def run_single_image(img_path, model, device, ps, show_grid, annotate_indices, overlay_alpha, max_side=None):
    img = load_image(img_path, max_side=max_side)
    ...
    def update(idx):
        ...
        disp = (cos_map - cos_map.min()) / (np.ptp(cos_map) + 1e-8)
        rgba = cmap(disp)
...
def run_two_images(img1_path, img2_path, model, device, ps, show_grid, annotate_indices, overlay_alpha, max_side=None):
    img1, img2 = load_image(img1_path, max_side=max_side), load_image(img2_path, max_side=max_side)
...
def main():
    ...
    parser.add_argument("--patch_size", type=int, default=(PATCH_SIZE_OVERRIDE or -1),
                        help="Override the model token stride (usually 16 for ViT-S/16, often 32 for ConvNeXt). Not a UI zoom.")
    parser.add_argument("--max_side", type=int, default=0,
                        help="Optional: resize input image(s) so max(width,height)=this before processing. "
                             "This makes patches easier to click on large images.")
    ...
    # `args.model` can be either a Hugging Face model ID OR a local folder path
    trust = bool(str(args.model).startswith("facebook/dinov3")) or os.path.isdir(str(args.model))
    model = _automodel_from_pretrained(args.model, trust_remote_code=trust).to(device)
    model.eval()
    ...
    max_side = int(args.max_side) if args.max_side and int(args.max_side) > 0 else None
    ...
    if img1 and img2:
        run_two_images(img1, img2, model, device, ps, show_grid, annotate_indices, overlay_alpha, max_side=max_side)
    else:
        img_single = img1 or DEFAULT_URL
        run_single_image(img_single, model, device, ps, show_grid, annotate_indices, overlay_alpha, max_side=max_side)
